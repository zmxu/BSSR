...
% R = [0.9731 -0.192 0.1275; 0.1956 0.9805 -0.01629; -0.1219 0.0479 0.9917];
R = R';
src = imread(filename);
% test
% src = rgb2gray(src);
% theta原图太大，总是内存不足，所以缩小0.5
...
V = sin(long).*cos(lati);
W = sin(lati);
% Test, to make sure long_new is right.

% lati_new = asin(W);
% long_sin = asin(V ./ cos(lati_new));
% long_cos = acos(U ./ cos(lati_new));
